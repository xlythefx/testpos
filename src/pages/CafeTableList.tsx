...
const statusColor = (s: string) => ({
  available: 'bg-green-100 dark:bg-green-950/30 text-green-700 dark:text-green-400',
  occupied: 'bg-red-100 dark:bg-red-950/30 text-red-700 dark:text-red-400',
  reserved: 'bg-yellow-100 dark:bg-yellow-950/30 text-yellow-700 dark:text-yellow-400',
}[s] ?? 'bg-stone-100 text-stone-500');

export function CafeTableList() {
  const [page, setPage] = useState(1);
  const [rows, setRows] = useState<CafeTable[]>([]);
  const [meta, setMeta] = useState<Paginated<CafeTable>['meta'] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    setError(null);
    try {
      const res = await apiGet<Paginated<CafeTable>>(`/cafe-tables?page=${p}`);
      setRows(res.data);
      setMeta(res.meta);
    } catch (e) {
      setError(String(e));
  ...
  const remove = async (id: number) => {
    ...
    await apiDelete(`/cafe-tables/${id}`);
    load(page);
  };
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100">Café Tables</h1>
        <Link to="/cafe-tables/new" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary hover:bg-amber-700 text-white text-sm font-medium transition-colors">
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 dark:bg-stone-950/50 text-stone-500">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-xs uppercase tracking-wider">Capacity</th>
            ...
            <tbody>
              {rows.length === 0 && <tr><td colSpan={4} className="text-center px-4 py-10 text-stone-400">No tables yet.</td></tr>}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-stone-100 dark:border-stone-800/60 hover:bg-stone-50 dark:hover:bg-stone-800/30">
                  ...
                  <td className="px-4 py-3 text-stone-600 dark:text-stone-400">{row.capacity} seats</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${statusColor(row.status)}`}>{row.status}</span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/cafe-tables/${row.id}`)} className="inline-flex items-center px-2 py-1 rounded hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-600 dark:text-stone-400">
