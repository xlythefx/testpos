...
export function MenuItemList() {
  ...
  const [meta, setMeta] = useState<Paginated<MenuItem>['meta'] | null>(null);
  ...
  const load = async (p: number) => {
    ...
    setError(null);
    try {
      const res = await apiGet<Paginated<MenuItem>>(`/menu-items?page=${p}`);
      setRows(res.data);
      setMeta(res.meta);
    } catch (e) {
      setError(String(e));
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-100">Menu Items</h1>
        <Link to="/menu-items/new" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary hover:bg-amber-700 text-white text-sm font-medium transition-colors">
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 dark:bg-stone-950/50 text-stone-500">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-xs uppercase tracking-wider">Price</th>
                <th className="text-left px-4 py-3 font-medium text-xs uppercase tracking-wider">Available</th>
                <th className="text-right px-4 py-3 font-medium text-xs uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={5} className="text-center px-4 py-10 text-stone-400">No menu items yet.</td></tr>
              )}
              {rows.map((row) => (
                <tr key={row.id} className="border-t border-stone-100 dark:border-stone-800/60 hover:bg-stone-50 dark:hover:bg-stone-800/30">
                  ...
                  <td className="px-4 py-3 font-medium text-stone-900 dark:text-stone-100">${Number(row.price).toFixed(2)}</td>
                  <td className="px-4 py-3">
                    {row.is_available ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <XCircle className="w-4 h-4 text-stone-400" />}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => nav(`/menu-items/${row.id}`)} className="inline-flex items-center px-2 py-1 rounded hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-600 dark:text-stone-400">
